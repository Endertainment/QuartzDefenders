...
public class StatsGUI {
    ...
    private void menuCorner() {
        Material z = DataAdapter.getGlassPaneByColor(DyeColor.BLUE);
        Material y = DataAdapter.getGlassPaneByColor(DyeColor.GRAY);
        int[] arg1 = {0, 2, 4, 6, 8, 18, 26, 36, 44, 46, 48, 50, 52}, arg2 = {1, 3, 5, 7, 9, 17, 27, 35, 45, 47, 49, 51, 53}; // z, y
        for (int a : arg1) {
            inventory.setItem(a, ItemUtil.newItem(" ", null, z, 1));
        }
        for (int b : arg2) {
            inventory.setItem(b, ItemUtil.newItem(" ", null, y, 1));
        }
    }
