...
public class GamesGUI {
    ...
    private void a() {
        ...
        for (ItemStack s : gamesIS.keySet()) {
            inventory.setItem(gamesIS.get(s), s);
        }
    }

    private void menuCorner() {
        DyeColor y = DyeColor.valueOf(Language.getString("GUI.games.glass_color_1"));
        DyeColor z = DyeColor.valueOf(Language.getString("GUI.games.glass_color_2"));
        if (y == null) {
            y = DyeColor.PURPLE;
        }
        if (z == null) {
